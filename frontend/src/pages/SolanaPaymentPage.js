...
const SolanaPaymentPage = () => {
  ...
  const successCountdownIntervalRef = useRef(null);
  ...
  const autoVerifyTransaction = async () => {
    ...
    try {
      ...
      // Kiểm tra kỹ response để tránh false positive
      if (response && response.data && response.data.verified === true) {
        // Dừng polling và QR countdown khi đã verify thành công
        if (pollingIntervalRef.current) {
          clearInterval(pollingIntervalRef.current);
          pollingIntervalRef.current = null;
        }
        ...
        setSuccess('Thanh toán đã được xác nhận tự động!');
        
        // Countdown 5 giây trước khi redirect
        setSuccessCountdown(5);
        const countdown = setInterval(() => {
          setSuccessCountdown(prev => {
            if (prev <= 1) {
              clearInterval(countdown);
              // Redirect về campaign
              if (campaignId) {
        ...
        successCountdownIntervalRef.current = countdown;
      } else {
        setCheckingStatus(false);
      }
    } catch (err) {
      // Nếu donation đã được processed (404), dừng polling
      if (err.response?.status === 404) {
        if (pollingIntervalRef.current) {
          clearInterval(pollingIntervalRef.current);
          pollingIntervalRef.current = null;
        }
        // Có thể donation đã được verify từ nơi khác
            ...
            <div className="transaction-verify-section">
              <div className="alert alert-success">
                ...
                {successCountdown !== null && successCountdown > 0 && (
                  <p>Trang sẽ tự động đóng sau {successCountdown} giây...</p>
                )}
              </div>
            </div>
          )}
